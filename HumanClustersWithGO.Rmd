...
suppressPackageStartupMessages({c(library(R.utils), library(dplyr), library(tidyr), 
  library(data.table), library(RColorBrewer), library(gplots), library(Polychrome), 
  library(edgeR), library(GO.db), library(gage), library(gProfileR), library(factoextra),
  library(NbClust), library(org.Sc.sgd.db), library(stringi))
})
...
cluster$ensembl_gene_id <- rownames(clusters)

# Grouping genes by cluster (using cluster 1)
cluster1 <- cluster[cluster$ClusterNumber == 1,] 
cluster1_list <- cluster1$ensembl_gene_id

# look for cluster gene match in GO dictionary and subset nonzero columns 
cluster1GO <- orgGOmatrixwide[orgGOmatrixwide$ensembl_gene_id %in% cluster1_list,] 
...
# Filter by row
cluster1inp <- orgGOmatrixwide[orgGOmatrixwide$ensembl_gene_id %in% rownames(inpClust1df),]
...
# Filter original GO matrix to include only the genes of interest and the GO terms from the imputed df
# Filter by row
cluster3inp <- orgGOmatrixwide[orgGOmatrixwide$ensembl_gene_id %in% rownames(impClust3df),]
