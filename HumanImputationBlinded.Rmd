...
# Normalised, aggregated, and scaled RNASeq counts - taken from DataPrep
normAggScale <- readRDS("normAggScale")
normAggLog <- readRDS("normAggLog")
# Binary dataframe for GO annotations - taken from GOPrep
GO_table <- readRDS("GO_table")
# From GO_table, selected randomly as training data - taken from GOPrep
...
# Coarse sweep of the following parameters: cluster total(taken from cluster cuts and aggregates) and thresholds (0.1 to 0.9, variable). 
#The length in cl_list is the minimum cluster size used in the dynamic tree cut. These values are saved in the cutClustvalues_dynamic variable. 
...
if (length(cl_list ==0 ) ) {  stop("extraction error") }
