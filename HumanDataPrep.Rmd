...
hClust <- as.data.frame(hClust, row.names = NULL, optional = FALSE, ...,
              cut.names = FALSE, col.names = names(nHcust), fix.empty.names = TRUE,
...
Removes data functions to improve memory storage
...
MAX = min(length(hClust$height),2000)
cutClustvalues <- cl_lengthCut(hr=hClust, min=50, max=MAX, interval=2)
...
cutClustvalues_dynamic <- cl_cut_dynamic(hr=hClust, cl=distClust, 
  min=4, max=302, interval=2)
...
cutClusterSideBar <- clusterCols[normAggScale[["cutClustvalues"]]]
