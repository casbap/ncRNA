...
normAgg <- readRDS("normAggScale.rds")
clusterPass <- readRDS("clusterPass.rds")
GO_table <- readRDS("GO_table.rds")
GOallClusters <-  readRDS("GOallClusters.rds")
corrAllClusters <-  readRDS("corrAllClusters.rds")
...
cluster1_list <- names(clusterPass[clusterPass==1])
...
weightGOallClusters <- impute(GOterms_perCl= GOterms_perCl,
    cor_edge_list = cor_edge_list  ,
    cl_GOall = GOallClusters, 
...
# Get the list of GO terms per cluster from the original db (before blinding) to
# use as a reference for filtering the relevant GO terms per cluster.
# This also makes sure that dimensions (number of columns) in all of the trials are equal.
#GOtermsperClust <- GO_list_perCl(weightGOallClusters, 100)
GOtermsperClust <- GO_per_cl_list(weightGOallClusters, clust_total)
...
nullOnt <- setdiff(unique(rownames(normAgg)),unique(GO_table$ensembl_gene_id))
length(nullOnt)
```
